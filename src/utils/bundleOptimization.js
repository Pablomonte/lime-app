...
export const createPluginLoader = () => {
    const pluginLoaders = {
        ...
        'lime-plugin-node-admin': () => import('plugins/lime-plugin-node-admin'),
        'lime-plugin-remotesupport': () => import('plugins/lime-plugin-remotesupport'),
        'lime-plugin-mesh-wide': () => import('plugins/lime-plugin-mesh-wide'),
        'lime-plugin-mesh-wide-config': () => import('plugins/lime-plugin-mesh-wide-config'),
        'lime-plugin-mesh-wide-upgrade': () => import('plugins/lime-plugin-mesh-wide-upgrade'),
        'lime-plugin-notes': () => import('plugins/lime-plugin-notes'),
        'lime-plugin-ground-routing': () => import('plugins/lime-plugin-ground-routing'),
        'lime-plugin-changeNode': () => import('plugins/lime-plugin-changeNode'),
        'lime-plugin-network-admin': () => import('plugins/lime-plugin-network-admin'),
...
export const bundleOptimizationTips = {
    ...
    checkThresholds(sizes) {
        ...
        if (sizes.main > this.thresholds.main) {
            warnings.push(`Main bundle too large: ${Math.round(sizes.main/1024)}KB > ${Math.round(this.thresholds.main/1024)}KB`);
        }
        
        if (sizes.vendor > this.thresholds.vendor) {
            warnings.push(`Vendor bundle too large: ${Math.round(sizes.vendor/1024)}KB > ${Math.round(this.thresholds.vendor/1024)}KB`);
        }
