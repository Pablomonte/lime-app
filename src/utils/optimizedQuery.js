...
export const useOptimizedQuery = (queryKeyOrFn, queryFn, options = {}) => {
    ...
    const queryKey = typeof queryKeyOrFn === 'function' ? queryKeyOrFn() : queryKeyOrFn;
...
export const useRealtimeQuery = (queryKey, queryFn, options = {}) => {
    const cacheConfig = createOptimizedQueryConfig('realtime-override', options);
    return useQuery(queryKey, queryFn, {
        ...cacheConfig,
...
export const createInvalidationHelper = (queryClient) => ({
    ...
    invalidateAfterSystemChange: () => {
        queryClient.invalidateQueries({ 
            predicate: (query) => {
                const key = query.queryKey[0];
                return ['system', 'lime-utils', 'changes-need-reboot'].includes(key);
            }
        });
    },
    ...
    invalidateAfterNetworkChange: () => {
        queryClient.invalidateQueries({ 
            predicate: (query) => {
                const key = query.queryKey[0];
                return ['lime-utils', 'iwinfo', 'check-internet', 'bat-hosts'].includes(key);
            }
        });
    },
});