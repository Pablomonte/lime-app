...
export const TIME_BASED_INVALIDATION = {
    ...
    "network-status": {
        ...
        patterns: ["check-internet", "iwinfo", "bat-hosts"],
    },

    // Metrics data for monitoring
    "metrics-refresh": {
        interval: 2 * 60 * 1000, // 2 minutes
        patterns: ["lime-metrics"],
    },
...
export class SmartInvalidator {
    ...
    invalidateByMutationType(mutationType, additionalPatterns = []) {
        ...
        console.log(
            `🔄 Invalidated queries for mutation type: ${mutationType}`,
            allPatterns
        );
    }
    ...
    invalidateRelated(primaryQueryKey, relatedPatterns = []) {
        ...
        performanceMonitor.trackInvalidation(
            `related:${primaryQueryKey.join(".")}`
        );
    }
    ...
    getMutationTypeFromKey(mutationKey) {
        const keyStr = Array.isArray(mutationKey)
            ? mutationKey[0]
            : String(mutationKey);
